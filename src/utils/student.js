...
export async function getMitudzCookies(username, password, isProxy = false, proxyPort = 7890) {
  // 1. 获取动态 AES 密钥
  ...
  if (isProxy) {
    ...
    loginConfig.httpsAgent = new HttpsProxyAgent(`http://127.0.0.1:${proxyPort}`);
    loginConfig.httpAgent = new HttpProxyAgent(`http://127.0.0.1:${proxyPort}`);
  }
