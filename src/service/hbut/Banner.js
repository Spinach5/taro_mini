...
import { extractBannerImages } from "../../utils/hbut/bannerHelper";
 ...
 */
export async function getBanner(forceRefresh = false) {
  // 1. 强制刷新时清除缓存
  if (forceRefresh) {
    ...
    if (cached) {
      console.log("[getBanner] 从缓存获取轮播图");
      return cached;
    }
  }
  const loginConfig = {
  ...
  try {
    // 2. 通过封装好的 request 请求学校主页 HTML（H5 走 Vite 代理，weapp 直连）
    const response = await createRequest.get(`${API_BASE.hbut_www}/`,loginConfig);
