...
export async function auth(stuID, password) {
  let encodedPassword
  try {
  ...
  try {
    ...
    // 1. 检查是否返回 JSON（失败情况）
    if (typeof responseData === 'object' || responseData?.startsWith('{') || responseData?.startsWith('[')) {
      try {
        const jsonData = typeof responseData === 'object' ? responseData : JSON.parse(responseData)

        // 根据返回的 JSON 判断失败原因
        if (jsonData.code !== 0 && jsonData.code !== 200) {
          console.error('登录失败:', jsonData.message || jsonData.msg || '未知错误')
          return {
            success: false,
