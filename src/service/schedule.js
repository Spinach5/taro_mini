...
export async function extractXhid() {
  const res = await request(`${API_BASE.hbut}/admin/pkgl/xskb/queryKbForXsd`, { responseType: 'text' })
  return extractXhidFromHtml(res.data)
}

/** 当前学年学期 */
export async function getCurrentSemester() {
  const res = await request(`${API_BASE.hbut}/v2/system/jcsj/xnxq/getCurrentXnxq?crossOrigin=true`, {
    method: 'POST',
...
export async function getUserInfo() {
  const res = await request(`${API_BASE.hbut}/v2/xjgl/xsjbxx/getXsjbxxByUser`, {
    method: 'POST',
...
export async function getUserUnit() {
  const res = await request(NOTEYD_BASE_URL + '/proxy/apis/proxy/proxyApiReq?uuid=uidbelongfids&crossOrigin=true&proxy_url=%2Fapi%2Fv2%2Fuidbelongfids', {
    method: 'POST',
    headers: { Referer: 'https://i.chaoxing.com' }
  })
...
export async function getStudentAcademic() {
  const res = await request(`${API_BASE.hbut}/v2/xsd/index/getStudentAcademicCompletionRate`, {
    method: 'POST',
    headers: { Referer: 'https://i.chaoxing.com' }
  })
  return res.data.code === 1 ? res.data.data : null
...
export async function getSemesterList() {
  const res = await request(`${API_BASE.hbut}/v2/system/jcsj/xnxq/getXnxqList?crossOrigin=true`, {
    method: 'POST',
    headers: { Referer: 'https://i.chaoxing.com' }
  })
  return res.data.code === 1 ? res.data.data : null
